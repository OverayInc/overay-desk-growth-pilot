...
- **제품 컨텍스트 반영 완료** — OVERAY DESK 실제 정보(Windows PC 연동 멀티 모니터 XR 워크스페이스, 가상 모니터 5개 무료, XR 펜/노트, 패스스루) 기준으로: 스토어 리스팅 3종을 실제 URL/ID로 시드(`live`), 기본 메일 템플릿 8종을 실제 피치로 교체(스팀 큐레이터 템플릿 삭제), 에이전트 프롬프트/발굴 기본 시드(VR 생산성·가상 모니터 키워드) 설정, 대시보드의 게임별 그로스 섹션 제거.
...
1. 대시보드 KPI — 기존 지표(위시리스트/판매/매출)는 Steam 데이터 기반이라 현재 0으로 표시됨. Meta/Play/Pico 스토어별 지표 소스(수동 입력? 콘솔 CSV? API?)를 정해야 대시보드를 살릴 수 있다.
2. 스토어별 키/프로모 코드 운영 방식 — 키 풀에 스토어 구분 필드를 붙일지, 라벨 컨벤션으로 충분한지.
3. 메일 발신 주소/서명 실제 값(현재 서명은 "— 오버레이(Overay Inc.) · Overay Desk 팀") 확인.

## 다음 단계 제안

1. 대시보드 지표를 새 스토어 기준으로 재설계 (미결 1)
2. Steam 백엔드 코드 일괄 제거 (routes/sync/CSV/테스트) — UI는 이미 분리되어 있어 독립 작업 가능
3. Facebook 페이지·Discord 채널 운영 로그/지표가 필요해지면 YouTube·Reddit 화면과 같은 패턴으로 추가
